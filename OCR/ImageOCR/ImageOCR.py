...
Tesseract OCRのWindowsインストーラをダウンロードしてインストール
        ...
        if image_path and save_path:
            try:
                ...
                text: str = pytesseract.image_to_string(
                    Image.open(image_path), lang=language, config=custom_config
                )
                ...
                if detected_encoding is None or result["confidence"] < 0.5:
                    ...
                    detected_encoding = (
                        "utf-8"  # デフォルトのエンコーディングをUTF-8に設定
                    )

                # 画像ファイルのベース名を取得し、テキストファイル名を作成
                base_name: str = os.path.splitext(os.path.basename(image_path))[0]
                text_file_name: str = os.path.normpath(
                    os.path.join(save_path, f"{base_name}.txt")
                )
                ...
                eg.popup(
                    f"OCR処理が完了しました。結果は '{text_file_name}' に保存されました。"
                )
            except FileNotFoundError:
                eg.popup_error(
                    "ファイルが見つかりません。正しいパスを指定してください。"
                )
            except UnidentifiedImageError:
                eg.popup_error(
                    "画像ファイルの読み込みに失敗しました。対応している画像形式か確認してください。"
                )
